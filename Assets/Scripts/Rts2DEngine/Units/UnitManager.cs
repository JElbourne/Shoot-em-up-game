...
using Rts2DEngine.MouseSelection;
using Rts2DEngine.Pathfinding;
...
namespace Rts2DEngine
{
    namespace Units
    {
        public class UnitManager : MonoBehaviour
        {
            ...
            {
                if (SelectionManager.instance.currentSelectedObjects.Count > 0)
                {
                    ...
                    if (Input.GetMouseButtonDown(1) && !buildMode)
                    {
                        ...
                        if (Physics.Raycast(ray, out hit, Mathf.Infinity))
                        {
                            if (hit.collider.gameObject.name == "Ground")
                            {
                                ...
                                originalRightClickPosition = hit.point;
            ...
            {
                List<GameObject> selectedUnits = SelectionManager.instance.currentSelectedObjects;
                for (int i = 0; i < selectedUnits.Count; i++)
                {
                    Vector3 newTarget = originalRightClickPosition;

                    if (!MapInstance.instance.NodeFromWorldPoint(newTarget).walkable || occupiedNodes.Contains(newTarget))
                    {
                    ...
                    else if (MapInstance.instance.NodeFromWorldPoint(newTarget).walkable && !occupiedNodes.Contains(newTarget))
                    {
            ...
            private Vector3 FindClosestAvailableNode(List<Vector3> listOfVectors)
            {
                ...
                {
                    if (MapInstance.instance.NodeFromWorldPoint(listOfVectors[i]).walkable && !occupiedNodes.Contains(listOfVectors[i]))
                    {
            ...
            private void RemoveFromTargetsAndMoveingUnits()
            {
                if (SelectionManager.instance.currentSelectedObjects.Count > 0)
                {
                    for (int i = 0; i < SelectionManager.instance.currentSelectedObjects.Count; i++)
                    {
                        SelectionManager.instance.currentSelectedObjects[i].GetComponent<Unit>().RemoveUnitFromUnitManagerMovingUnitsList();
                    }
                }
            }
        }
